    ...
    function statusBackground(monthTotalDays, salary, travel, other) {
        ...
        for(let i=0; i<statusDivs.length; i++){
            ...
            if(content == "Present") {
                ...
                if (stats.includes(statusDivs[i-1].innerText.trim()) || stats.includes(statusDivs[i+1].innerText.trim())) {
                    presentDaysCount += 0;
                } else {
                    presentDaysCount += 1;
                }
            } else if (content == "Holiday" && statusDivs[--i].innerText.trim() == "NA" && statusDivs[++i].innerText.trim() == "NA"){
                presentDaysCount -= 1;
            }
        }
        var calSalary = ((Number(salary)+Number(travel)+Number(other))/monthTotalDays)*presentDaysCount;
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    for(let y=1; y<=daysInSelectedMonth; y++){
                        ...
                        const annualHolidays = {Jan:[14,15,26], Feb:[19], Mar:[8,24,25], Apr:[9,14,17], May:[1], Jun:[28,29], Aug:[15,19,26], Sep:[7], Oct:[2,9,12,31], Nov:[2], Dec:[25]};
                    ...
                    statusBackground(daysInSelectedMonth, res[1].empSalary, res[1].empTravelAll, res[1].empOtherAll);
                },
                error: function (err) {
                    console.log(err);
                }
